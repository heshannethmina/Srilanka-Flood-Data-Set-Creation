...
and leakage-free train/val/test splits.

Flood state = GloFAS river discharge >= per-node return-period threshold
(estimated on the TRAIN period only). This is the operational definition used
by systems like GloFAS itself (return-period exceedance), so labels are
physically grounded rather than synthetic.

Output: data/processed/flood_dataset.parquet  (analysis-ready, one row/node-day)
...
def add_flood_state(df, thresholds):
    ...
    df["flood_state"] = df[f"flood_{C.PRIMARY_SEVERITY}"]   # primary binary state

    # label confidence: low inside a +/-15% ambiguity band around primary thr
    thr_p = df["node_id"].map(lambda n: thresholds[n][C.PRIMARY_SEVERITY])
    rel = (df["river_discharge"] - thr_p).abs() / thr_p.replace(0, np.nan)
    conf = np.clip(rel / 0.15, 0, 1)                 # 0 at threshold, 1 outside band
    conf = 0.5 + 0.5 * conf                          # floor 0.5
    df["label_confidence"] = np.where(df["river_discharge"].isna(), 0.0, conf).astype("float32")
    return df


def add_horizon_targets(df):
    """Predict whether the cell will be in flood state within the next h days."""
    parts = []
    for nid, g in df.groupby("node_id", sort=False):
        ...
        for h in C.FLOOD_HORIZONS_DAYS:
            ...
            # onset = flood in window AND not currently flooding
            g[f"target_onset_{h}d"] = np.where(
                (fut == 1) & (state == 0), 1.0, np.where(np.isnan(fut), np.nan, 0.0))
        # regression targets: next-day discharge & next-3d max z-score
...
def add_splits(df):
    ...
    df["split_basin_holdout"] = np.where(df["basin"] == C.HOLDOUT_BASIN, "test", "train")
    return df


def add_validity(df):
    """Mark rows usable as supervised samples (enough history + defined target)."""
    df = df.sort_values(["node_id", "date"]).reset_index(drop=True)
    warm = df.groupby("node_id").cumcount() >= 30      # 30-day feature warmup
...
def main():
    ...
    print(f"Final dataset: {df.shape[0]} node-days ({v.shape[0]} valid samples) "
          f"x {df.shape[1]} cols")
    print(f"Flood events: {len(events)}  | positive rate (24h target, valid) = "
          f"{v['target_flood_1d'].mean():.4f}")
    print(f"Splits (valid): "
          f"{v['split_temporal'].value_counts().to_dict()}")
    print(f"-> {out}")
