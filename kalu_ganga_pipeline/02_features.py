"""
Step 2 - Assemble per-node daily panel and engineer features.

Reads cached raw parquet from data/raw/, merges weather + discharge + ERA5 precip
per node, and computes dynamic (time-varying) and static features.

Output: data/processed/panel_features.parquet   (long format: one row per node-day)
        data/processed/thresholds.json           (per-node flood thresholds)
"""
import os
import sys
import json
...
def load_node_frame(nid):
    ...
    # high-res ERA5 precipitation (preferred over POWER's coarser 0.5deg precip)
...
def discharge_features(df):
    ...
    # Seasonal climatology fitted on TRAIN only (no leakage)
    ...
    df["discharge_pctl"] = (np.searchsorted(train_q_sorted, q.to_numpy(), side="right")
                            / max(len(train_q_sorted), 1))
...
def build():
    ...
    for nid, nd in nodes.items():
        ...
        if df["river_discharge"].notna().sum() < 365:
            print(f"  WARN: {nid} has insufficient discharge data, skipping")
            continue
        df["node_id"]  = nid
        df["basin"]    = nd["basin"]
        df["zone"]     = nd["zone"]
